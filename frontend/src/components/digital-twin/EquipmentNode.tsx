...
const typeConfig: Record<string, { icon: string; border: string; bg: string; glow: string }> = {
  ...
  synthesiser:   { icon: "🧪", border: "border-cyan-500/60",   bg: "bg-cyan-500/10",   glow: "glow-blue"  },
  characteriser: { icon: "⚡", border: "border-yellow-500/60", bg: "bg-yellow-500/10", glow: "glow-amber" },
...
export const EquipmentNode = memo(function EquipmentNode({ data }: NodeProps) {
  ...
  return (
    <div className={cn(
      ...
      {(d.failProb !== undefined || d.timeCostS !== undefined) && (
