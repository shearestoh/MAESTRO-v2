  ...
  return (
    <div className="glass-panel p-4 space-y-2" style={{ userSelect: "none", position: "relative" }}>
      ...
      <div className="flex items-center gap-2 text-xs">
        <button
          onClick={jumpToNow}
          className="px-2 py-0.5 rounded font-medium bg-slate-100 text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition-colors"
        >
          ↩ Now
        </button>
        {actualEntries.length > 0 && (
          <button
            onClick={fitHistory}
            className="px-2 py-0.5 rounded bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors"
          >
            ⊡ Fit history
          </button>
        )}
      ...
      <div className="flex items-center gap-4 text-[10px] text-slate-500 pt-1">
        ...
        <div className="flex items-center gap-1.5">
          <div
            className="w-5 h-2.5 rounded opacity-55"
            style={{
              backgroundColor: "#a5f3fc",
              backgroundImage: "repeating-linear-gradient(90deg,transparent,transparent 3px,rgba(255,255,255,.5) 3px,rgba(255,255,255,.5) 6px)",
            }}
          />
          <span>Projected</span>
        </div>
